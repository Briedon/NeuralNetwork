...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.igraf=torch.FloatTensor(n_nodes,maximumLine+1).cuda()
        for i in range(n_nodes):
            for j in range(len(inputgraf[i])):
                print(self.igraf[i][j])
                print(inputgraf[i][j])
    ...
    def forward(self, x):
        ...
        for neuron in self.neurons:
            ...
            if ind[i] == 0:
                #print(a)
                inp=Variable(torch.FloatTensor([x[a]])).cuda()
                #print(inp)
                values[i]=F.relu(neuron(inp))
                a=a+1
            elif ind[i]>1:
                l = torch.cat([values[j] for j in self.igraf[i]],0)
                #print(l)
                # print(inp)
